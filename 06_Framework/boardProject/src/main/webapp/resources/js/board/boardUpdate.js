...
// -> X 버튼 클릭 시 순서를 한번만 저장하는 용도

// -> 위에서 얻어온 요소들의 개수가 같음 == 인덱스가 일치함
for(let i=0; i<inputImage.length; i++){
    inputImage[i].addEventListener('change', e => { // 파일이 선택되거나 선택 후 취소되었을 때
        ...
        if(file != undefined){ // 파일이 선택되었을 때
            ...
            reader.readAsDataURL(file); // 지정된 파일을 읽은 후 result 변수에 URL 형식으로 지정
        ...
        } else { // 선택 후 취소되었을 때 -> 선택된 파일 없음 -> 미리보기 지움
...
const boardWriteFrm = document.querySelector('#boardUpdateFrm');
...
boardUpdateFrm.addEventListener('submit', e => {
    ...
    // input type="hidden" 태그에 deleteSet의 저장된 값을 "1,2,3" 형태로 변경해서 저장

    // Array.from(deleteSet): Set -> Array 변경
    // JS 배열은 String에 대입되거나 출력될 때 요소, 요소, 요소 형태의 문자열을 반환
