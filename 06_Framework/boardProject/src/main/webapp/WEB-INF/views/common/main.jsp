    ...
    <main>
        ...
        <section class="content">
            <section class="content-1">
                ...
                <h3>이메일을 입력받아 일치하는 회원의 정보를 모두 조회</h3>
                    ...
                    <c:when test="${empty sessionScope.loginMember}">
                        <form action="/member/login" method="post" id="loginFrm">
                            ...
                            <label>
                                ...
                                <%-- 위 코드를 아래처럼 응용해서 개량할 수도 있음 --%>

                                <c:if test="${not empty cookie.saveId.value}">
                                    <%-- cookie에 저장된 이메일이 있으면 save 변수 선언 
                                        -> pageScope(페이지 내에서 사용 가능, if문 끝나도 가능)
                                    --%>
                    ...
                    <c:otherwise>
                        <article class="login-area">
                            <a href="/myPage/profile">
                                <c:if test="${empty loginMember.profileImage}"> <%-- 프로필 이미지가 없으면 기본 이미지 --%>
                                    <img src="/resources/images/user.png" id="memberProfile">
                                </c:if>
                                <c:if test="${!empty loginMember.profileImage}"> <%-- 프로필 이미지가 있으면 있는 이미지 --%>
