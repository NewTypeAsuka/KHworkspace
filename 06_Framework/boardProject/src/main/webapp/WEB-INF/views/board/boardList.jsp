...
<c:set var="boardName" value="${map.boardTypeList[boardCode-1].BOARD_NAME}"/>
...
<body>
    <main>
        ...
        <c:if test="${not empty param.key}">
            <c:set var="sp" value="&key=${param.key}&query=${param.query}"/> <%-- sp로 변수 선언 --%>
        </c:if>
        ...
        <section class="board-list">
            ...
            <div class="list-wrapper">
                <table class="list-table">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty boardList}"> <%-- 조회된 게시글 목록이 비어있거나 null인 경우 --%>
                            ...
                            <c:otherwise>
                                <c:forEach items="${boardList}" var="board">
                                    <tr> <!-- 게시글 목록 조회 결과가 있지 않다면 -->
                                        <td>${board.boardNo}</td>
                                        <td> 
                                            <c:if test="${not empty board.thumbnail}"> <%-- 썸네일이 있을 경우 --%>
                                                <img class="list-thumbnail" src="${board.thumbnail}">
                                            </c:if>

                                            <%-- ${boardCode}: @pathvariable로 requestScope에 추가된 값 --%>
