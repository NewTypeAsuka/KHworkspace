...
//		System.out.println("암호화 확인: " + bcrypt.encode(inputMember.getMemberPw()));
		...
		if(loginMember != null) { // 아이디가 일치하는 회원이 조회된 경우
			...
			// 입력한 비밀번호, 암호화된 비밀번호가 같은지 확인
			if(bcrypt.matches(inputMember.getMemberPw(), // 같은 경우
								loginMember.getMemberPw())) {
				
				loginMember.setMemberPw(null); // 비밀번호를 유지하지 않기 위해서 로그인 정보에서 제거
	...
	// 예외가 발생하면 rollback, 발생 안하면 Service 종료 시 commit
