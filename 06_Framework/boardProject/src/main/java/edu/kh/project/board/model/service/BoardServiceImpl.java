...
public class BoardServiceImpl implements BoardService {
	...
	public Map<String, Object> selectBoardTypeList(int boardCode, int cp) {
		...
		// 2. 1번 조회 결과 + cp를 이용해서 Pagination 객체 생성 -> 내부 필드가 모두 계산되어 초기화
		Pagination pagination = new Pagination(listCount, cp);
		
		// 3. 특정 게시판에서 현재 페이지에 해당하는 부분에 대한 게시글 목록 조회
		// (어떤 게시판(boardCode)에서 몇 페이지(pagination.currentPage)에 대한 게시글 몇개 조회)
		List<Board> boardList = dao.selectBoardTypeList(pagination, boardCode);
	...
	public int like(Map<String, Integer> paramMap) {
		...
		if(paramMap.get("check") == 0){ // 좋아요 상태 X
			...
			result = dao.insertBoardLike(paramMap); // BOARD_LIKE 테이블 INSERT
			
		} else { // 좋아요 상태 O
			
			result = dao.deleteBoardLike(paramMap); // BOARD_LIKE 테이블 DELETE
			
		}
		
		// SQL 수행 결과가 0 == DB 또는 파라미터에 문제가 있음
		// 1) 에러를 나타낼 수 있는 임의의 값을 반환 (-1)
		// 2) 사용자 정의 예외 발생
	...
	public Map<String, Object> selectBoardList(Map<String, Object> paramMap, int cp) {
		...
		// 3. 특정 게시판에서 현재 페이지에 해당하는 부분에 대한 게시글 목록 조회 + 검색 조건이 일치하는 글만
