...
// @RequestMapping("요청 주소") -> GET/POST 구분 없이 모두 받음
...
public class MemberController {
	...
	// -> MemberService를 구현한 MemberServiceImpl에 Bean 주입
	@Autowired 
	...
	// 클래스에 작성한 /member를 제외한 나머지 부분을 주소로 작성
...
//	@PostMapping("/login")
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// 파라미터 전달 방법 3: @ModelAttribute 이용하는 방법
		
		// - DTO(또는 VO)와 같이 사용하는 어노테이션
		// - 전달받은 파라미터의 name 속성 값이 같이 사용되는 DTO에 필드명과 같다면
	...
	public String login(Member inputMember, Model model, 
		...
		// @RequestHeader(value="referer") String referer
		// -> 요청 HTTP Header에서 "referer"(이전 주소) 값을 얻어와 매개 변수 String referer에 저장
		
		// Model: 데이터 전달용 객체
		// -> 데이터를 K : V 형식으로 담아서 전달
		// -> 기본적으로 requestScope
		// -> @SessionAttributes 어노테이션과 함께 사용 시 SessionScope
...
//		System.out.println(loginMember);
		...
		if(loginMember != null) { // 로그인 성공
			...
			model.addAttribute("loginMember", loginMember); // 현재는 requestScope
			// 2) 클래스 위에 @SessionAttributes 추가 -> sessionScope로 변경
			...
			if(saveId != null) { // 체크되었을 때
				...
				// 0초 동안 유지되는 쿠키 생성 -> 기존 쿠키를 삭제
				cookie.setMaxAge(0);
				
			}
			// 클라이언트가 어떤 요청을 할 때 쿠키가 첨부될 지 경로(주소)를 지정
			cookie.setPath("/"); // localhost/ 이하 모든 주소	 ex) /, /member/login, /member/logout 등
			 ...
			 * 
			 * RedirectAttributes:
			 * - 리다이렉트 시 데이터를 requestScope로 전달할 수 있게 하는 객체
			 * 
			 * 응답 전: requestScope
			 * 응답 중: sessionScope로 잠시 이동
			 * 응답 후: requestScope로 복귀
			 */
	...
	public String logout(SessionStatus status, HttpSession session) {
		...
		// 2) Model + @SessionAttributes를 이용한 경우 -> SessionStatus.setComplete()
...
//		session.invalidate();
   ...
   public String signUp(Member inputMember, String[] memberAddress, RedirectAttributes ra) {
	   ...
	   // RedirectAttributes ra: 리다이렉트 시 데이터를 requestScope로 전달하는 객체
	   // -------------------------------------------------------------
...
//	   inputMember.setMemberAddress(addr);
	   ...
	   if(inputMember.getMemberAddress().equals(",,")) {
		   ...
		   // String.join("구분자", String[]) -> 배열의 요소를 하나의 문자열로 변경(요소 사이에 구분자 추가)
	   ...
	   if(result > 0) { // 가입 성공 시 메인 페이지로
		   path += "/"; 
		   message = inputMember.getMemberNickname() + "님의 가입을 환영합니다.";
	   } else { // 가입 실패 시 회원 가입 페이지로
		   path += "/member/signUp";
		   message = "회원 가입이 실패했습니다.";
	   }
	   
	   // 리다이렉트 시 session에 잠깐 올라갓다 내려오도록 세팅
	...
	/*
	 * Spring 예외 처리 방법(3종류, 우선 순위, 중복 사용 가능)
	 * 
	 * 1순위: 메서드 단위로 처리
	 * 		-> try-catch / throws
	 * 2순위: 클래스 단위로 처리
...
//	@ExceptionHandler(Exception.class)
	public String exceptionHandler(Exception e, Model model) {
		...
		// Model model: 데이터 전달용 객체(requestScope가 기본)
