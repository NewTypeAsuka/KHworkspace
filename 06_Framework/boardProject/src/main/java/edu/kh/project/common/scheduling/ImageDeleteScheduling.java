...
// 스프링이 일정 시간마다 해당 객체를 이용해서 코드를 수행 == 스프링이 해당 클래스를 객체로 만들어서 관리해야 함 == bean 등록
@Component // @Controller, @Service, @Repository의 부모 어노테이션
			// Bean 등록만을 위한 명시하는 어노테이션(역할 X)
public class ImageDeleteScheduling {
	...
	// 일 5초 -> 대기 10초 -> 출력(일 시작 후 15초 뒤)
	@Scheduled(fixedDelay = 10000) // ms 단위
	public void test() {
		System.out.println("스케줄러가 10초마다 자동으로 출력"); // 백엔드 콘솔에서 출력 -> 프론트는 interval 함수
	}
	...
	// 일 5초
	// 대기 10초 -> 출력(일 시작 후 10초 뒤)
	public void text2() {
		System.out.println("스케줄러가 10초마다 자동으로 출력");
	}
	
	// - cron 속성 : UNIX계열 잡 스케쥴러 표현식으로 작성 - cron="초 분 시 일 월 요일 [년도]" - 요일 : 1(SUN) ~ 7(SAT) 
//	@Scheduled(cron = "0,30 * * * * * ") // -> 매분 0초, 30초 실행한다는 뜻
	@Scheduled(cron = "0 0 * * * * * ") // -> 매 정시마다 실행한다는 뜻( *시 0분 0초 )
	public void text3() {
//		System.out.println("스케줄러가 x분 0초, x분 30초마다 자동으로 출력");
		
		System.out.println("-----게시판 DB, 서버 불일치 파일 제거-----");
		// 서버에 저장된 파일 목록을 조회해서 DB에 저장된 파일 목록과 비교하여 매칭되지 않는 서버 파일 제거
		
		// 1. 서버에 저장된 파일 목록 조회 -> application 객체를 이용해서 /resources/images/board의 실제 서버 경로를 얻어옴
...
//				System.out.println(server.getName());
				...
				if(dbImageList.indexOf(server.getName()) == -1) { // 서버 파일 이름에 있는 게 db파일 목록에 없는 경우
 ...
 *     
 *  - cron 속성 : UNIX계열 잡 스케쥴러 표현식으로 작성 - cron="초 분 시 일 월 요일 [년도]" - 요일 : 1(SUN) ~ 7(SAT) 
 * 	ex) 2019년 9월 16일 월요일 10시 30분 20초 cron="20 30 10 16 9 2 2019" // 연도 생략 가능
 * 
 * - 특수문자 * : 모든 수.
