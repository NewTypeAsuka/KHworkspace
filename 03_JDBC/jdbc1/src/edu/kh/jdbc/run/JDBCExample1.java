...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// DB 연결 정보를 담는 객체 -> Connection을 상속받아 구현한 자식 객체 대입될 예정
		...
		//    DBMS마다 알맞게 구현하고 나머지는 구현된 객체를 Java에서 참조해서 사용
		
		Statement stmt = null;
		// sql을 DB에 전달해 수행하고 결과(Result Set 또는 DML 성공 행의 개수)를 반환받는데
		// 사용되는 객체
		...
		try {
			...
			// -> OracleDriver.class를 객체로 만들어 두기
			
			Class.forName("oracle.jdbc.driver.OracleDriver");
			// -> () 안에 작성된 경로에 존재하는 클래스를 이용해 객체를 생성
			// -> 메모리에 객체가 생성되고 Connection을 이용한 DB 연결시 이를 자동으로 참조함
			
			// ClassNotFoundException 발생 가능성 있음 -> 오타 또는 라이브러리 추가 안했을 때 발생
			
			// 2. DB 연결 정보를 담은 Connection 객체 얻어오기
			// -> DriverManager를 이용해서 생성
			
			// DB 연결을 위한 정보를 변수에 저장
			String type = "jdbc:oracle:thin:@"; // 드라이버 종류
			String ip = "localhost"; // localhost == 127.0.01.1 (loop back ip == 자기 컴퓨터 주소)
			String port = ":1521"; // 컴퓨터 내 응용프로그램 번호
			String dbName = ":ORCL"; // DB 이름(비슷한 단어로 SID) 19c 버전은 :ORCL / 18c 버전은 :EX
			String user = "kh_lsj"; // 계정명
			String pw = "oracle"; // 비밀번호
			
			// DriverManager 객체
			// - 메모리에 로드된 JDBC 드라이버 객체를 이용해 DB에 접근하고
...
//			System.out.println(conn);
			...
			while(rs.next()) {
				...
				// 현재 커서가 접근한 행의 EMP_ID.. 컬럼의 값을 얻어와 String..으로 반환
