...
public class MainView {
			...
			try {
				if(Session.loginMember == null) { // 로그인 X
					...
					case 3: System.out.println("\n=== 로그아웃 되었습니다 ====\n");
						Session.loginMember = null; // 참조하고 있던 로그인 회원 객체를 제거
						break;
					case 0: System.out.println("\n=== 프로그램 종료 ===\n"); break;
					default: System.out.println("\n*** 메뉴 번호만 입력해주세요 ***\n");
					}
	...
	private void login() {
		...
		try { // 로그인 서비스 호출 후 결과 반환 받기 -> 반환 결과는 Session.loginMember에 저장
	...
	private void signUp() {
		...
		try {
			...
			while(true) {
				...
				memberId = sc.next();
				// 아이디 중복 확인 서비스 호출 -> 중복인 경우 1, 아닌 경우 0 반환
