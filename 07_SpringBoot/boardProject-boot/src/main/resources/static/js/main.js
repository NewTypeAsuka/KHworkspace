...
loginFrm.addEventListener("submit", e => {
    ...
    if(memberEmail.value.trim().length == 0){
        ...
        memberEmail.focus(); // 이메일 input 태그에 커서 두기
    ...
    if(memberPw.value.trim().length == 0){
        ...
        memberPw.focus(); // 비밀번호 input 태그에 커서 두기
...
btn1.addEventListener("click", () => {
    ...
    .then(resp => resp.text())
    // resp: 응답 객체
    // resp.text(): 응답 객체를 문자열로 변환하여 반환

    .then(tel => {

        /* 비동기 요청 후 수행할 코드 */
        result1.innerText = tel; // 조회할 결과를 result1에 출력
    })
...
function sendMessage(name, str){
    ...
    testSock.send(JSON.stringify(obj)); // 웹소켓 연결된 곳으로 메시지 전달
                    // JS객체 -> JSON
