...
-- 기존 INNER JOIN 결과에 JOIN 구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서
-- JOIN 결과에 포함도지 않은 행들도 조회하도록 함

-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
-- 왼쪽 테이블 컬럼의 값과 오른쪽 테이블 컬럼의 값이 일치하지 않아도 억지로 추가
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 하나의 테이블을 가지고 생각하지 말고 같은 테이블이 두 개 있다고 생각
...
SELECT EMP_NAME, JOB_NAME, SALARY,
	NVL((SALARY+(SALARY*BONUS))*12, SALARY*12) AS "ANNUAL SALARY"
FROM EMPLOYEE
NATURAL JOIN JOB
NATURAL JOIN SAL_GRADE
WHERE NVL(SALARY+(SALARY*BONUS), SALARY) > MIN_SAL
ORDER BY EMP_ID;
...
-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 부서가 같은 동료를 조회하시오.
-- SELF JOIN 사용
SELECT E1.EMP_NAME, DEPT_CODE, E2.EMP_NAME 
FROM EMPLOYEE E1
JOIN EMPLOYEE E2 USING (DEPT_CODE)
WHERE NOT E1.EMP_ID = E2.EMP_ID
ORDER BY 1;
