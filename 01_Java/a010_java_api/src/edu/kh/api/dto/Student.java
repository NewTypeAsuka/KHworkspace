...
public class Student {
	...
	// Object.equals()
	// -> 현재 객체와 다른 객체가 동등(필드 값이 같은지)한지 비교
	// -> 필드를 가지고 있는 자식 클래스가 알맞은 형태로 재정의(오버라이딩)
	@Override
	public boolean equals(Object obj) {
		...
		// 비교를 위해 전달받은 객체 Student가 맞는가? -> false인 경우에 수행
	...
	// - hash 함수: 입력받은 문자열/숫자를 특정한 문자열/숫자로 반환 -> 최대한 중복되지 않는 숫자를 만듦
	// - hashCode(): 객체의 필드 값을 이용해서 일정한 길이의 숫자를 만드는 함수
	// 왜 필요한가? -> Java 실행 시 내부에서 객체 검색 속도 증가
	// 어떻게 작성하는가? -> 필드 값이 같으면 항상 같은 수가 나올 수 있도록 구현
