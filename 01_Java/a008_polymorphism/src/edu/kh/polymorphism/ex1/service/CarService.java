...
public class CarService {
	...
	// 다형성: 객체가 다양한 형태를 지니는 것
	// - 상속을 이용한 기술로 부모 타입의 참조 변수 하나로 여러 타입의 자식 객체를 참조할 수 있음
	
	public void ex1() {
		// 부모 참조 변수 = 부모 객체
		Car c1 = new Car();
		
		/* 업 캐스팅: 자식 객체 -> 부모 객체로 변함 */
		// -> 자식 객체 내부에 있는 부모 객체를 잠조하도록 변함
		
		// 부모 참조 변수 = 자식 객체
		Car c2 = new Truck();
		Car c3 = new LightCar();
		
		// Truck 객체가 Car로부터 상속받은 메서드 사용
		c2.setWheel(10);
		c2.setSeat(3);
		c2.setFuel("경유");
...
//		c2.setWeight(2.5); // c2로 접근 불가능하여 에러
	...
	public void ex2() {
		...
		// LightCar로 강제 형변환 -> c1이 LightCar 객체를 모두 참조 가능
		((LightCar)c1).print();
...
//		c1.getDiscountOffer();
	...
	public void ex3() {
		// 다형성(업 캐스팅, 다운 캐스팅) + 객체배열
		
		// Car 1차원 배열 == Car 참조변수의 묶음
		Car[] carList = new Car[3];
		
		// carList[i] == Car 참조변수
		carList[0] = new Truck(6, 4, "경유", 1); // 업 캐스팅
		carList[1] = new LightCar(4, 4, "가솔린", 0.3); // 업 캐스팅
		...
		for(Car c : carList) {
			...
			// Truck일 경우 -> 최대 적재 하중: 2.5ton
			// LightCar일 경우 -> 할인율: 30%
			// Car일 경우 -> 차종이 등록되어있지 않습니다
			
			/* instanceof 연산자 */
			// - 참조하는 객체의 타입을 검사하는 연산자
			// ex) 참조변수a instanceof 클래스명 -> 참조변수가 참조하는 객체 타입과 클래스 타입이 일치하면 true 아니면 false
			if(c instanceof Truck) { // c가 참조하는 객체가 Truck 객체인 경우
				System.out.println("최대 적재 하중: " + ((Truck)c).getWeight() + "ton");
				// 만약 다운 캐스팅이 잘못될 경우 ClassCastException 발생
				// ex) Light 객체를 Truck 참조변수가 참조하려고 할 때
				// 해결방법: instanceof를 통해 참조하는 객체의 자료형을 판단하여 적절한 형식으로 다운 캐스팅
			}else if(c instanceof LightCar) { // c가 참조하는 객체가 LightCar 객체인 경우
				System.out.println("할인율: " + ((LightCar)c).getDiscountOffer()*100 + "%");
			}else { // c가 참조하는 객체가 Truck도 LightCar도 아닌 경우
	...
	public void ex4() {
		// 다형성(업 캐스팅, 다운 캐스팅) + 객체배열 + 매개변수에 다형성 + 바인딩(정적/동적) + instanceof
		// Car 1차원 배열 == Car 참조변수의 묶음
		Car[] carList = new Car[3];
	...
	public void printCar(Car c) {
		...
		// 바인딩: 메서드 호출 구문과 실제로 수행될 메서드를 연결하는 것
		// 정적 바인딩
		// - "프로그램 실행 전" 컴파일 단계에서 메서드 호출부와 수행될 메서드를 묶는 것
		// - 참조 변수의 자료형을 기준으로 연결
		// 동적 바인딩
		// - "프로그램 실행 중" 실행 단계에서 메서드와 후행될 메서드를 묶는 것
